...
export const createNote = asyncHandler(async (req, res) => {
  ...
  res
    .status(201)
    .json(
      new ApiResponse(200, { content: newContent }, "Message sent successfully")
    );
});
...
export const deleteNote = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(201, {}, "Note deleted"));
});
...
export const fetchAllNotesByGroup = asyncHandler(async (req, res) => {
  ...
  res
    .status(201)
    .json(
      new ApiResponse(200, { content: notes }, "Notes fetched successfully")
    );
});
export const fetchAllNotes = asyncHandler(async (req, res) => {
  ...
  // if no available note it throws an error
  if (notes.length === 0) {
    throw new ApiError(400, "No messages found!!... Start adding some");
  }
  res
    .status(201)
    .json(
      new ApiResponse(200, { content: notes }, "Messages fetched successfully")
    );
});
...
export const approveNote = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new ApiResponse(200, { content: note }, "Note approved"));
});
...
export const generateContent = asyncHandler(async (req, res) => {
  ...
  res
    ...
    .json(
      new ApiResponse(
        200,
        { title, content: generatedContent },
